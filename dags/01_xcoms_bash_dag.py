...
@dag(dag_id="xcoms_bash_dag", catchup=False)
def xcoms_bash_dag():
    ...
    def bash_task():
        return """echo "This is a bash task!. The fetched data is {{ ti.xcom_pull(key='fetch_data', task_ids='fetch_data') }}" """

    fetch_data() >> process_data() >> bash_task()

first_dag_instance = xcoms_bash_dag()